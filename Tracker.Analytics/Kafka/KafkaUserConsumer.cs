...
public class KafkaUserConsumer : BackgroundService
{
    ...
    {
        var topics = new[] { this._userWasAddedTopic, this._userWasUpdatedTopic, this._userWasDeletedTopic, this._instructionWasAddedTopic, this._instructionWasUpdatedTopic };
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                else if (consumeResult.Topic == _instructionWasUpdatedTopic)
                {
                    var kafkaInstruction = JsonSerializer.Deserialize<KafkaInstruction>(payload)!;
                    await instructionService.UpdateInstruction(kafkaInstruction);
                }
                else if (consumeResult.Topic == _instructionWasAddedTopic)
                {
                    var kafkaInstruction = JsonSerializer.Deserialize<KafkaInstruction>(payload)!;
                    await instructionService.InsertInstruction(kafkaInstruction);
                }
            }
